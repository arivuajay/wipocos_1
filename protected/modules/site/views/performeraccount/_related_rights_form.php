                            ...
                            <div class="box-body">
                                <div class="col-lg-12">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="row">
                                            <?php
                                            if ($model->Perf_Rel_Managed_Rights_Id && is_array($model->Perf_Rel_Managed_Rights_Id)) {
                                                $selected_keys = array_flip($model->Perf_Rel_Managed_Rights_Id);
                                            ...
                                            echo CHtml::dropDownList('Perf_Rel_Managed_Rights_Id', array(), $remain_manages, array('class' => 'form-control', 'multiple' => true, 'id' => 'select-perf-rel-mgmd-from', 'size' => 7));
                                            echo '</div><div class="col-sm-2 mt30"><button type="button" id="btn-add-perf-rel-mgmd-select" class="btn btn-default btn-sm">>></button><br />';
                                            echo '<br /><button type="button" id="btn-remove-perf-rel-mgmd-select" class="btn btn-default btn-sm"><<</button></div><div class="col-sm-5">';
                                            echo CHtml::dropDownList('Perf_Rel_Managed_Rights_Id', array(), $selected_manages, array('class' => 'form-control', 'multiple' => true, 'id' => 'select-perf-rel-mgmd-to', 'size' => 7));

                                            echo $form->dropDownList($model, 'Perf_Rel_Managed_Rights_Id', $managed_rights, array('class' => 'hide', 'multiple' => 'multiple'));
                                            echo '</div>';
                                            ?>
                                        </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="row">
                                            <?php
                                            if ($model->Perf_Rel_Territories_Id && is_array($model->Perf_Rel_Territories_Id)) {
                                                $selected_terr_keys = @array_flip($model->Perf_Rel_Territories_Id);
                                            ...
                                            echo '<div class="col-sm-5">';
                                            echo CHtml::dropDownList('Auth_Mnge_Territories_Source', array(), $remain_terr, array('class' => 'form-control', 'multiple' => true, 'id' => 'select-perf-rel-terr-from', 'size' => 7));
                                            echo '</div><div class="col-sm-2 mt30"><button type="button" id="btn-add-perf-rel-terr-select" class="btn btn-default btn-sm">>></button><br />';
                                            echo '<br /><button type="button" id="btn-remove-perf-rel-terr-select" class="btn btn-default btn-sm"><<</button></div><div class="col-sm-5">';
                                            echo CHtml::dropDownList('Auth_Mnge_Territories_Destination', array(), $selected_terr, array('class' => 'form-control', 'multiple' => true, 'id' => 'select-perf-rel-terr-to', 'size' => 7));

                                            echo $form->dropDownList($model, 'Perf_Rel_Territories_Id', $territories, array('class' => 'hide', 'multiple' => 'multiple'));
                                            echo '</div>';
                                            ?>
                                        </div>
...
$js = <<< EOD
    $(document).ready(function(){
        $('#btn-add-perf-rel-mgmd-select').click(function(){
            $('#select-perf-rel-mgmd-from option:selected').each( function() {
                $('#select-perf-rel-mgmd-to').append("<option value='"+$(this).val()+"'>"+$(this).text()+"</option>");
                $('#PerformerRelatedRights_Perf_Rel_Managed_Rights_Id option[value="'+$(this).val()+'"]').attr('selected','selected');
                $(this).remove();
            });
            return false;
        });
        $('#btn-remove-perf-rel-mgmd-select').click(function(){
            $('#select-perf-rel-mgmd-to option:selected').each( function() {
                $('#select-perf-rel-mgmd-from').append("<option value='"+$(this).val()+"'>"+$(this).text()+"</option>");
                $('#PerformerRelatedRights_Perf_Rel_Managed_Rights_Id option[value="'+$(this).val()+'"]').removeAttr('selected','selected');
                $(this).remove();
            });
            return false;
        });
        $('#btn-add-perf-rel-terr-select').click(function(){
            $('#select-perf-rel-terr-from option:selected').each( function() {
                $('#select-perf-rel-terr-to').append("<option value='"+$(this).val()+"'>"+$(this).text()+"</option>");
                $('#PerformerRelatedRights_Perf_Rel_Territories_Id option[value="'+$(this).val()+'"]').attr('selected','selected');
                $(this).remove();
            });
            return false;
        });
        $('#btn-remove-perf-rel-terr-select').click(function(){
            $('#select-perf-rel-terr-to option:selected').each( function() {
                $('#select-perf-rel-terr-from').append("<option value='"+$(this).val()+"'>"+$(this).text()+"</option>");
                $('#PerformerRelatedRights_Perf_Rel_Territories_Id option[value="'+$(this).val()+'"]').removeAttr('selected','selected');
                $(this).remove();
            });
            return false;
        });
